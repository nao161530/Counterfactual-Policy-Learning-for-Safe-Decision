...
mlp = MLP1(input_size = x_tr.shape[1])
mlp.fit(x_tr[t_tr == 1], (y_tr[t_tr == 1]/(p_tr[t_tr == 1]) - y1_or[t_tr == 1]/(p_tr[t_tr == 1])) + y1_or[t_tr == 1], batch_size = 10)
y1_dr = mlp.predict(x_tr)



with open("data/IHDP/ihdp1_train_data", "wb") as file:
...
print("数据已成功保存到文件 'data/IHDP/ihdp1_train_data'")
# print("数据已成功保存到文件")
print("done")

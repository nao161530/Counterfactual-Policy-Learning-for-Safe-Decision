...
class model_cate_se(nn.Module):
    def __init__(self, input_size):
        super(model_cate_se, self).__init__()
...
class model_lagrange_se(nn.Module):
    def __init__(self, input_size):
        super(model_lagrange_se, self).__init__()
        self.input_size = input_size
        self.model = MLP(input_size=self.input_size)
        self.xent_func = torch.nn.BCELoss()
    ...
    def fit(self, x, t, y, p, mu0, mu1, y0_dr, y1_dr, c, rho, harm_num, thr=-5, stop=10, num_epoch=100, batch_size=20, panelty=0, lr=0.01,
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                harm_loss = torch.sum(((1 - sub_t) * (sub_y - sub_mu0) / (1 - sub_p) + sub_mu0) * (1 - sub_mu1) * pred -
                                      (sub_t * (sub_y - sub_mu1) / sub_p) * sub_mu0 * pred)
...
class model_constraint_se(nn.Module):
    def __init__(self, input_size):
        super(model_constraint_se, self).__init__()
        self.input_size = input_size
        self.model = MLP(input_size=self.input_size)
        self.xent_func = torch.nn.BCELoss()
    ...
    def predict(self, x):
        ...
        pred = np.random.binomial(1, pred)
        return pred
class model_linear_se(nn.Module):
    def __init__(self, input_size):
        super(model_linear_se, self).__init__()
        self.input_size = input_size
        self.model = MLP(input_size=self.input_size)
        self.xent_func = torch.nn.BCELoss()
